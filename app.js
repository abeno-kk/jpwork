...
/* ===== 多分頁任務勾選同步 + 防覆蓋修正 ===== */
(function () {
  const TASK_KEY = 'office_task_checkbox_state_v2';
  ...
  function getRowId(el) {
    ...
    return tr?.dataset?.id || tr?.rowIndex || el.dataset.id || '';
  }

  function getDate(el) {
    return el.dataset.dateKey || el.getAttribute('data-date-key') || '';
  }
  ...
  function restoreCheckboxes() {
    ...
    document.querySelectorAll('input[type="checkbox"][data-date-key]').forEach(el => {
      const id = getRowId(el);
      const date = getDate(el);

      if (all[id] && typeof all[id][date] === 'boolean') {
        el.checked = all[id][date];
      }
    });
  }
  ...
  document.addEventListener('change', function(e) {
    ...
    if (el && el.matches && el.matches('input[type="checkbox"][data-date-key]')) {
      saveCheckbox(el);
  ...
  function delayedRestore() {
    ...
    setTimeout(restoreCheckboxes, 300);
    setTimeout(restoreCheckboxes, 800);
  }
  ...
  window.addEventListener('beforeunload', saveAllCheckboxes);
